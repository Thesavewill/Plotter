...
object GridDrawer {
    ...
    private fun DrawScope.drawGridWithLabels(
        canvasState: CanvasTransform, width: Float, height: Float, lineWidth: Float
    ) {
        ...
        for (i in startIdxX..endIdxX) {
            ...
            if (isMajor) {
                // Крупная линия
                drawLine(majorColor.toComposeColor(), Offset(px, 0f), Offset(px, height), lineWidth)
                ...
                // Мелкая линия
                drawLine(minorColor.toComposeColor(), Offset(px, 0f), Offset(px, height), lineWidth * 0.7f)
            }
        }
        ...
        for (i in startIdxY..endIdxY) {
            ...
            if (isMajor) {
                // Крупная линия
                drawLine(majorColor.toComposeColor(), Offset(0f, py), Offset(width, py), lineWidth)
                ...
                // Мелкая линия
                drawLine(minorColor.toComposeColor(), Offset(0f, py), Offset(width, py), lineWidth * 0.7f)
            }
        }
    }
    ...
    private fun DrawScope.drawAxes(
        ...
        if (canvasState.offsetY in 0f..height) {
            drawLine(axisColor.toComposeColor(), Offset(0f, canvasState.offsetY), Offset(width, canvasState.offsetY), lineWidth)
            drawContext.canvas.nativeCanvas.drawText("x", width - 20f, canvasState.offsetY - 8f, labelPaint)
        }

        if (canvasState.offsetX in 0f..width) {
            drawLine(axisColor.toComposeColor(), Offset(canvasState.offsetX, 0f), Offset(canvasState.offsetX, height), lineWidth)
            drawContext.canvas.nativeCanvas.drawText("y", canvasState.offsetX + 18f, 22f, labelPaint)
        }
    }

    // Вспомогательная функция для конвертации Int цвета в Compose Color
