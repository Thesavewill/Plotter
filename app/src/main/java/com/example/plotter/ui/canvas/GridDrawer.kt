...
object GridDrawer {
    private val lineColor1 = Color(0xFFCCCCCC)
    private val lineColor2 = Color(0xFF888888)
    private val lineColor3 = Color.Black
    ...
    private fun DrawScope.drawGridWithLabels(
        ...
        for (i in startIdxX..endIdxX) {
            ...
            if (isMajor) {
                drawLine(lineColor2, Offset(px, 0f), Offset(px, height), lineWidth)
                ...
                drawLine(lineColor1, Offset(px, 0f), Offset(px, height), lineWidth * 0.7f)
            }
        }
        ...
        for (i in startIdxY..endIdxY) {
            ...
            val isMajor = (i % minorLinesPerMajor == 0)
            if (isMajor) {
                drawLine(lineColor2, Offset(0f, py), Offset(width, py), lineWidth)
                ...
                drawLine(lineColor1, Offset(0f, py), Offset(width, py), lineWidth * 0.7f)
            }
        }
    }
    ...
    private fun DrawScope.drawAxes(
        ...
        if (canvasState.offsetY in 0f..height) {
            drawLine(lineColor3, Offset(0f, canvasState.offsetY), Offset(width, canvasState.offsetY), lineWidth)
            drawContext.canvas.nativeCanvas.drawText("x", width - 20f, canvasState.offsetY - 8f, labelPaint)
        }
        if (canvasState.offsetX in 0f..width) {
            drawLine(lineColor3, Offset(canvasState.offsetX, 0f), Offset(canvasState.offsetX, height), lineWidth)
